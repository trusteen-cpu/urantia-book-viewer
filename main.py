...
input_ref = st.text_input("Enter reference (e.g. 111:7 or 111:7.5)", "")

# --- 검색 로직 ---
if input_ref:
    input_ref = input_ref.strip()
    # 장 전체 검색 or 특정 절 검색 처리
    results = [key for key in ko_texts.keys() if key.startswith(input_ref)]
    
    if results:
        # 🔹 절 번호 숫자 기준 정렬
        results = sorted(results, key=lambda x: tuple(map(int, re.findall(r'\d+', x))))
